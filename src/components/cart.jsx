...
const Viewcart = ({
  ...
  function decrement(id,originalquantity){
    ...
    setcart((cart) =>
      cart.map((itm) => (id === itm.id ? { ...itm, quantity: originalquantity-1 } : itm))
    );
  }
              ...
              <div className={cartmodule.highlighted}>
                ...
                {/* input elment */}
                   <div className= {cartmodule.PlusMinusContainer}>
                    <button className={cartmodule.controlbtn+" btn"} onClick={()=>{decrement(item.id,(item.quantity<1)?item.quantity=1:item.quantity)}}>-</button>
                    <p>{item.quantity}</p>
                    <button className={cartmodule.controlbtn+" btn"}  onClick={()=>{increment(item.id,item.quantity)}}>+</button>
                   </div>
                </div>
                <div>
                <span className="rate ">Rs.{isNaN( item.rate * item.quantity)? 0:item.rate * item.quantity}/-</span>
                </div>
              </div>
              <button
 ...
 {(length<1 && tot===0)?<p></p>: <div className="d-flex justify-content-between align-items-center">
